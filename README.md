...
- **失败可重试**：某次翻译失败后，再次右键即可重试，无需重启 Zotero
- **零配置凭证**：OCR Token、模型 API Key 自动读取 RetainPDF 桌面配置，可覆盖
  （在插件设置里配齐模型三件套后，桌面配置损坏也不影响翻译）
- **自动拉起服务**：本地 API（127.0.0.1:41000）没启动时自动启动 RetainPDF.exe
  （支持绿色安装，自动扫描各盘符 `X:\RetainPDF\RetainPDF.exe`）
- 翻译进度实时显示在 Zotero 右下角；后端瞬时抖动会自动重试，不会让任务白跑
...
- **译文里中文只有一部分 / 全是英文原文**：翻译任务成功但模型侧没有返回译文
  ...
  RetainPDF 桌面版配置。
- **在 PDF 里加了批注/高亮，为什么没有自动重新翻译**：Zotero 的批注是独立元数据，
  不会改写 PDF 文件本身，所以不触发重翻（避免同一份 PDF 反复重翻、白花模型费）。
  需要更新译文时，右键条目选「RetainPDF 重新翻译（删除旧译文后重翻）」。
- **翻译失败后想重试**：直接再右键一次「RetainPDF 翻译全文并挂到条目」即可；
